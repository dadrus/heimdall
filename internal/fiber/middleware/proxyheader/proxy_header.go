...
func New() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		forwardedForHeaderValue := x.IfThenElse(c.IsProxyTrusted(), c.Get(headerXForwardedFor), "")
		forwardedHeaderValue := x.IfThenElse(c.IsProxyTrusted(), c.Get(headerForwarded), "")
		clientIP := c.IP()
