...
func TestNewScope(t *testing.T) {
	...
	for uc, tc := range map[string]struct {
		...
		"creates rule scoped resolver with namespace": {
			setup: func(t *testing.T, bindings *BindingProviderMock) *scope {
				t.Helper()

				releaser := NewScopeReleaserMock(t)

				return newScope(bindings,
					withID("foo"),
					withReleaser(releaser),
	...
	for uc, tc := range map[string]struct {
		...
		"returns secret set handle and tracks lease": {
			setup: func(t *testing.T, bindings *BindingProviderMock) *scope {
				...
				bdg := newScopeTestBinding(t, key, secrets)
	...
	for uc, tc := range map[string]struct {
		...
		"returns credentials handle and tracks lease": {
			setup: func(t *testing.T, bindings *BindingProviderMock) *scope {
				...
				bdg := newScopeTestBinding(t, key, creds)
	...
	for uc, tc := range map[string]struct {
		...
		"returns certificate bundle handle and tracks lease": {
			setup: func(t *testing.T, bindings *BindingProviderMock) *scope {
				...
				bdg := newScopeTestBinding(t, key, bundle)
...
func TestScopeUsesRuleScopedReferences(t *testing.T) {
	t.Parallel()
	...
	scp := newScope(bindings, withID("foo"),
		withReleaser(releaser),
		withNamespace("team-a"),
	)
...
func TestScopeRelease(t *testing.T) {
	...
	for uc, tc := range map[string]struct {
		setup  func(t *testing.T, bindings *BindingProviderMock, releaser *ScopeReleaserMock, calls *guardedScopeCalls) *scope
		assert func(t *testing.T, scp *scope, calls *guardedScopeCalls)
	}{
		"runs cleanups releases bindings and notifies releaser": {
			setup: func(
				t *testing.T,
				bindings *BindingProviderMock,
				releaser *ScopeReleaserMock,
				...
				releaser.EXPECT().
					releaseScope("foo", mock.AnythingOfType("*secrets2.scope")).
					Run(func(_ string, _ *scope) {
						calls.Add("release:scope")
					})
		...
		t.Run(uc, func(t *testing.T) {
			...
			scp := tc.setup(t, bindings, releaser, &calls)
