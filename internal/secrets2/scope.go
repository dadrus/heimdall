...
func withReleaser(releaser scopeReleaser) scopeOption {
	return func(s *scope) {
		s.releaser = releaser
	}
}
...
type scopeReleaser interface {
	releaseScope(id string, scope *scope)
}

type scope struct {
	bindings   bindingProvider
	releaser   scopeReleaser
...
func (s *scope) Release() {
	...
	if s.releaser != nil {
		s.releaser.releaseScope(s.id, s)
	}
}
...
var (
	...
	_ RuleScopedResolver = (*scope)(nil)
