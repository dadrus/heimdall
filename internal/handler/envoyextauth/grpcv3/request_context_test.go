...
func TestNewRequestContext(t *testing.T) {
	...
	require.Equal(t, "moo", ctx.Request().QueryParameter("bar"))
	require.Equal(t, httpReq.RawBody, ctx.Request().Body())
	require.Empty(t, ctx.Request().FormParameter("foo"))
	require.Equal(t, "heimdall", ctx.Request().FormParameter("content"))
