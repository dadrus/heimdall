...
func New(logger zerolog.Logger) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		accLog := createAccessLogger(logger, start, traceCtx)
		accLog.Info().Msg("TX started")

		res, err := handler(ctx, req)

		createAccessLogFinalizationEvent(ctx, accLog, err, start, traceCtx).Msg("TX finished")

		return res, err
	}
}

func createAccessLogger(logger zerolog.Logger, start time.Time, traceCtx *tracecontext.TraceContext) zerolog.Logger {
