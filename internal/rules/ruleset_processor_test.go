...
func TestRuleSetProcessorOnCreated(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			ruleset: &config.RuleSet{Version: RuleSetVersion, Rules: []config.Rule{{ID: "foo"}}},
			configureFactory: func(t *testing.T, mhf *mocks.FactoryMock) {
				t.Helper()
		...
		},
		{
			...
			configureFactory: func(t *testing.T, mhf *mocks.FactoryMock) {
				...
				mhf.EXPECT().CreateRule(RuleSetVersion, mock.Anything, mock.Anything).Return(&mocks.RuleMock{}, nil)
			},
			assert: func(t *testing.T, err error, queue event.RuleSetChangedEventQueue) {
				t.Helper()
