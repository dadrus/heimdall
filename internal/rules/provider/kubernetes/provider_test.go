...
type RuleSetResourceHandler struct {
	...
	rsCurrent v1alpha2.RuleSet

	rsUpdatedEvt chan v1alpha2.RuleSet
	rsCurrentEvt chan v1alpha2.RuleSet

	updateStatus func(rs v1alpha2.RuleSet, callIdx int) (*metav1.Status, error)
...
func (h *RuleSetResourceHandler) handle(t *testing.T, r *http.Request, w http.ResponseWriter) {
	...
	case r.URL.Path == "/apis/heimdall.dadrus.github.com/v1alpha2/rulesets":
		h.listCallIdx++
...
func (h *RuleSetResourceHandler) writeWatchResponse(t *testing.T, w http.ResponseWriter) {
	...
	h.rsCurrent = *wEvt.Object.(*v1alpha2.RuleSet) // nolint: forcetypeassert
...
func (h *RuleSetResourceHandler) writeListResponse(t *testing.T, w http.ResponseWriter) {
	...
	rsl := v1alpha2.RuleSetList{
		TypeMeta: metav1.TypeMeta{
			APIVersion: fmt.Sprintf("%s/%s", v1alpha2.GroupName, v1alpha2.GroupVersion),
			Kind:       "RuleSetList",
		},
		ListMeta: metav1.ListMeta{ResourceVersion: "735820"},
		Items:    []v1alpha2.RuleSet{rs},
	}
...
func TestProviderLifecycle(t *testing.T) {
	for _, tc := range []struct {
		...
		assert         func(t *testing.T, statusList *[]*v1alpha2.RuleSetStatus, processor *mocks.RuleSetProcessorMock)
	}{
		{
			uc:   "rule set added",
			...
			assert: func(t *testing.T, statusList *[]*v1alpha2.RuleSetStatus, processor *mocks.RuleSetProcessorMock) {
				...
				assert.Equal(t, "1/1", (*statusList)[0].UsedBy)
		...
		},
		{
			...
			assert: func(t *testing.T, statusList *[]*v1alpha2.RuleSetStatus, processor *mocks.RuleSetProcessorMock) {
				...
				assert.Len(t, *statusList, 1)
				assert.Equal(t, "0/1", (*statusList)[0].UsedBy)
		...
		},
		{
			...
			assert: func(t *testing.T, statusList *[]*v1alpha2.RuleSetStatus, processor *mocks.RuleSetProcessorMock) {
				...
				assert.Equal(t, "1/1", (*statusList)[1].UsedBy)
		...
		},
		{
			...
			assert: func(t *testing.T, statusList *[]*v1alpha2.RuleSetStatus, processor *mocks.RuleSetProcessorMock) {
				...
				assert.Equal(t, "0/1", (*statusList)[1].UsedBy)
				assert.Len(t, (*statusList)[1].Conditions, 1)
				condition = (*statusList)[1].Conditions[0]
				assert.Equal(t, metav1.ConditionFalse, condition.Status)
				assert.Equal(t, v1alpha2.ConditionRuleSetActivationFailed, v1alpha2.ConditionReason(condition.Reason))
		...
		t.Run(tc.uc, func(t *testing.T) {
			...
			handler := &RuleSetResourceHandler{
				rsUpdatedEvt: make(chan v1alpha2.RuleSet, 2),
				rsCurrentEvt: make(chan v1alpha2.RuleSet, 2),
