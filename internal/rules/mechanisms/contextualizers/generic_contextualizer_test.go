...
func TestNewGenericContextualizer(t *testing.T) {
	t.Parallel()
...
`),
		...
		t.Run(uc, func(t *testing.T) {
			...
			// THEN
			contextualizer, ok := mech.(*genericContextualizer)
			if err == nil {
				require.True(t, ok)
			}
...
func TestGenericContextualizerCreateStep(t *testing.T) {
	t.Parallel()
...
`),
		...
		t.Run(uc, func(t *testing.T) {
			...
			configured, ok := mech.(*genericContextualizer)
			require.True(t, ok)
...
`)
			...
			configureContext: func(t *testing.T, ctx *pipelinemocks.ContextMock) {
				...
				reqf := pipelinemocks.NewRequestFunctionsMock(t)
		...
		t.Run(uc, func(t *testing.T) {
			...
			configureCache := x.IfThenElse(tc.configureCache != nil,
				tc.configureCache,
				func(t *testing.T, _ *mocks.CacheMock, _ *genericContextualizer, _ pipeline.Subject) {
					t.Helper()
				})
