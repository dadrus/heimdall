...
func (r *Rule) IsMatching(strategy config.MatchingStrategy, method string, u *url.URL) (bool, error) {
...
func (r *Rule) ReplaceAllString(strategy config.MatchingStrategy, input, replacement string) (string, error) {
	if err := ensureMatchingEngine(r, strategy); err != nil {
		return "", err
	}
...
func ensureMatchingEngine(rule *Rule, strategy config.MatchingStrategy) error {
	...
	return fmt.Errorf("%w: strategy %s is unknown", ErrUnknownMatchingStrategy, string(strategy))
}

// ExtractRegexGroups returns the values matching the rule pattern
func (r *Rule) ExtractRegexGroups(strategy config.MatchingStrategy, u *url.URL) ([]string, error) {
	if err := ensureMatchingEngine(r, strategy); err != nil {
		return nil, err
	}
