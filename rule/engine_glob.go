...
func NewGlobEngine() MatchingEngine {
	e := new(globMatchingEngine)
...
func (ge *globMatchingEngine) compile(pattern string) error {
	if checksum := crc64.Checksum([]byte(pattern), ge.table); checksum != ge.checksum {
		compiled, err := ge.compileGlob(pattern, '<', '>')
		if err != nil {
			return err
		}
...
func (ge *globMatchingEngine) delimiterIndices(s string, delimiterStart, delimiterEnd rune) ([]int, error) {
	var level, idx int
	idxs := make([]int, 0)
...
func (ge *globMatchingEngine) compileGlob(pattern string, delimiterStart, delimiterEnd rune) (glob.Glob, error) {
	// Check if it is well-formed.
	idxs, errBraces := ge.delimiterIndices(pattern, delimiterStart, delimiterEnd)
	if errBraces != nil {
