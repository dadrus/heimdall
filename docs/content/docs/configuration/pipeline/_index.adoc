...
[source, javascript]
----
heimdall.log("This statement is only present in the logs, if the log level is set to debug")

return `{
  "subject_id": {{ quote heimdall.Subject.ID }},
  "email": {{ quote heimdall.Subject.Attributes.email }},
  "request_url": {{ quote heimdall.RequestURL }},
  "request_method": {{ quote heimdall.RequestMethod }},
  "x_foo_value": {{ heimdall.RequestHeader "X-Foo" | quote }}
}`
----
