...
Some pipeline handlers support, respectively require access to different types of objects, they work on, e.g. to render a header with specific values, or to check whether some expectations apply. Following objects are available and have the following structure:

=== Subject

This object contains the information about the authenticated subject. If available, it has the following attributes:
...
* *`attributes`*: _map_
+
Contains all attributes, which are known about the subject. The content is initially set by the authenticator, which was able to authenticate the subject. Handlers which receive the subject can update it, but do never override any entries.

Each object of this type can be thought as a JSON object and is also accessible as such to the corresponding handlers. Here some examples:

.Subject object created by an AnonymousAuthenticator
====
[source, json]
----
{
...
.Possible Subject object created by an OAuth2Authenticator
====
[source, json]
----
{
...
This object contains information about the request being authenticated and has the following attributes:
...
* *`url`*: _URL_
+
The URL of the matched request. This object has the following properties:
...
** *`query`*: _map_
+
The query part of the url with each key-value pair being a string to array of strings mapping.

* *`client_ips`*: _string array_
+
The list of IP addresses the request passed through with the first entry being the ultimative client of the request. Only available if heimdall is configured to trust the client, sending this information, e.g. in the `X-Forwarded-From` header (see e.g. Decision Service link:{{< relref "/docs/configuration/services/decision_api.adoc#_trusted_proxies" >}}[trusted_proxies] configuration for more details).

* *`headers`*: _map_
+
The headers available in the request with each key-value pair being a string to array of strings mapping.
...
----
{
  ...
  "client_ips": ["127.0.0.1", "10.10.10.10"],
  "headers": {
    "X-Bar":["foo"],
    "X-Foo":["bar","baz"]
  }
...
This example shows also the aforesaid mapping for `query` and `headers` attributes.

====

=== Payload

This object represents the contents of a payload, like the request body or a response body. The contents depend on the MIME-Type of the payload. For `json`, `yaml` or `x-www-form-urlencoded` encoded payload, the object is structured as JSON object. Otherwise, it is just a string.

Here some examples:

.Structured payload (response from OPA)
====
[source, json]
----
{ "result": true }
...
.Unstructured payload (response from OPA)
====
[source, text]
----
SomeStringValue
