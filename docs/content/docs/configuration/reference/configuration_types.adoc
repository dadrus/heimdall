...
CAUTION: Heimdall does not verify the integrity of the pipeline handler identifier configured using `raised_by` on start or while loading rules. Thus, if there is no pipeline handler with the given identifier, the error will not match.
