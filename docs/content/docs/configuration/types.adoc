...
+
NOTE: If https://datatracker.ietf.org/doc/html/rfc9449[DPoP] validation is enabled using `proof_of_possession` (see below), these algorithms are also used to validate DPoP proof JWT signatures.
...
+
NOTE: If DPoP validation is enabled using `proof_of_possession` (see below), this leeway is also considered while validating the `iat` claim of the DPoP proof JWT.

* *`proof_of_possession`*: _link:{{< relref "#_proof_of_possession_assertion" >}}[Proof of Possession Assertion]_ (optional)
+
Configures proof-of-possession validation for access tokens. If configured, heimdall enforces the configured proof-of-possession assertion. Otherwise, proof-of-possession validation is performed opportunistically. That means, if e.g., an access token is recognized as DPoP-bound, heimdall tries to satisfy the DPoP requirements according to https://www.rfc-editor.org/rfc/rfc9449[RFC 9449].
...
* *`secret`*: _link:{{< relref "/docs/operations/secret_management.adoc#_secret_references" >}}[Secret Reference]_ (mandatory)
+
...
* *`credentials`*: _link:{{< relref "/docs/operations/secret_management.adoc#_secret_references" >}}[Secret Reference]_ (mandatory)
+
...
* *`attributes`*: _string or map_ (optional)
+
If this property is defined as a single string, it must be a https://github.com/tidwall/gjson/blob/master/SYNTAX.md[GJSON Path] pointing to the attributes of the principal in the JSON object. Defaults to `@this`. Otherwise, if it defined as a key-value map, the values of that map must be https://github.com/tidwall/gjson/blob/master/SYNTAX.md[GJSON Path] expressions.
...
.Extracting principal from an https://www.ory.sh/docs/kratos/[Ory Kratos] "whoami" endpoint response
====
...
* `type` - The type of the proof of possession assertion.
...
This assertion validates DPoP-bound access tokens according to https://www.rfc-editor.org/rfc/rfc9449[RFC 9449].
...
`type` must be set to `dpop`. `config` supports the following properties:
...
* *`nonce_required`*: _boolean_ (optional)
+
Whether the DPoP proof must contain a nonce previously requested by heimdall. Defaults to `false`.
+
If set to `true`, heimdall requires a link:{{< relref "/docs/operations/secret_management.adoc#_master_key" >}}[master key] to generate and validate DPoP nonces. If DPoP nonce validation is enabled and the master key is not configured, heimdall refuses to start or rejects rules which apply the configuration at rule level.

* *`replay_allowed`*: _boolean_ (optional)
+
...
This type configures how heimdall signals token usage errors for authenticators using OAuth2 access tokens according to https://www.rfc-editor.org/rfc/rfc6750[RFC 6750], https://www.rfc-editor.org/rfc/rfc9449[RFC 9449], and related specifications.
...
+
For bearer token usage, the challenge uses the `Bearer` authentication scheme. For DPoP tokens, the challenge uses the `DPoP` authentication scheme.

* *`realm`*: _string_ (optional)
+
The realm to include in the `WWW-Authenticate` header. Defaults to `"Please authenticate"`.

* *`error_uri`*: _URI_ (optional)
+
A URI identifying a human-readable page with information about the error.

* *`include_error_description`*: _boolean_ (optional)
+
Whether to include the optional `error_description` parameter. Defaults to `false`.
+
CAUTION: Enabling this option may disclose implementation details and should be used with care.

* *`include_required_scope`*: _boolean_ (optional)
+
...
+
CAUTION: Enabling this option may disclose details about the protected resource's authorization model.

.Token usage error signaling configuration and possible challenges
====
[source, yaml]
----
enabled: true
