...
== Bearer Token Usage Error Signaling

This type configures how heimdall signals errors for authenticators using OAuth2 Bearer tokens according to https://www.rfc-editor.org/rfc/rfc6750[RFC 6750].

Following properties are available:

* *`enabled`*: _boolean_ (optional)
+
Enables the generation of the `WWW-Authenticate` response header using the `Bearer` authentication scheme. Defaults to `false`.
...
.Bearer token usage error signaling configuration
====
[source, yaml]
----
enabled: true
