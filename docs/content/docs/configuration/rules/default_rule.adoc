...
+
Which handlers to use to authenticate, authorize, hydrate (enrich) and mutate the subject of the request. At least one link:{{< relref "/docs/configuration/pipeline/authenticators.md" >}}[authenticator] and one link:{{< relref "/docs/configuration/pipeline/mutators.md" >}}[mutator] must be defined. A specific rule (see also link:{{< relref "rule_configuration.adoc" >}}[Rule Definition]) can omit the definition of authenticators, if it wants to reuse the authenticators defined in the default rule. Same is true for other handlers. Exception are link:{{< relref "/docs/configuration/pipeline/authorizers.md" >}}[authorizers] and link:{{< relref "/docs/configuration/pipeline/hydrators.md" >}}[hydrators]. As soon, as a specific rule defines at least one authorizer or hydrator, those authorizers and hydrators (defined in the default rule) are not considered anymore.
