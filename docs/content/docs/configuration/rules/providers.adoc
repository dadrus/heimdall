...
* `gc` for Google Cloud Storage and
* `azblob` for Microsoft Azure blobs
...
The last one instructs heimdall to load rule set from a specific blob, namely a blob named `my-rule-set`, which resides on the `my-bucket` AWS S3 bucket.
