...
** *`with`*: _ConnectionConfiguration_ (optional)
+
Can be used to specify connection specific properties. As of now, these are only timeouts.

*** *`timeout`*: _Timeout_ (mandatory)
+
Which `read` and `write` timeouts should be applied to the opened upstream connections. If configured at least one timeout must be defined. Both timeouts default to timeouts defined for the _link:{{< relref "/docs/configuration/services/proxy.adoc#_timeout" >}}[proxy service]_.
...
+
.Disable write timeouts for SSE use cases
====
[source, yaml]
----
id: rule:sse
forward_to:
  host: sse-backend:8080
  with:
    timeout:
      write: 0s
# further properties
