...
+
Defines how to match a rule. Can be a URL to match as glob pattern, or if the matching strategy is required to be set, is then based on the following properties:
...
The `match` property above is a short form for:
...
----
- id: rule:1
  match: https://my-service1.local/<**>
  methods: [ "GET" ]
  execute:
    - authorizer: foobar
- id: rule:2
  match: https://my-service2.local/<**>
  methods: [ "GET" ]
  execute:
    - authorizer: barfoo
----
