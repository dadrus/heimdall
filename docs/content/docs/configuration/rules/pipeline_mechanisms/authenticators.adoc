...
As the name implies, this authenticator does nothing. It tells heimdall to bypass the authentication. This is the only one authenticator type, which does not create a subject object on a successful execution, required by the most others pipeline mechanisms. This authenticator type also doesn't have any configuration options.
...
This authenticator verifies the provided credentials according to the HTTP "Basic" authentication scheme, described in https://datatracker.ietf.org/doc/html/rfc7617[RFC 7617]. This authenticator does not challenge the authentication, it only verifies the provided credentials and sets the subject id to the configured value if the authentication succeeds. Otherwise, it raises an error, resulting in the execution of the configured error handlers. The link:{{< relref "error_handlers.adoc#_www_authenticate" >}}["WWW Authenticate"] error handler mechanism can then for example be used if the corresponding challenge is required.
...
* *`identity_info_endpoint`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_endpoint">}}[Endpoint]_ (mandatory, not overridable)
+
...
* *`authentication_data_source`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_authentication_data_source" >}}[Authentication Data Source]_ (mandatory, not overridable)
+
...
* *`subject`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_subject" >}}[Subject]_ (mandatory, not overridable)
+
...
* *`session_lifespan`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_session_lifespan" >}}[Session Lifespan]_ (optional, not overridable)
+
...
* *`introspection_endpoint`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_endpoint">}}[Endpoint]_ (mandatory, not overridable)
+
...
* *`token_source`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_authentication_data_source" >}}[Authentication Data Source]_ (optional, not overridable)
+
...
* *`assertions`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_assertions" >}}[Assertions]_ (mandatory, overridable)
+
...
* *`subject`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_subject" >}}[Subject]_ (optional, not overridable)
+
...
* *`jwks_endpoint`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_endpoint">}}[Endpoint]_ (mandatory, not overridable)
+
...
* *`jwt_source`*: _link:{{< relref "/docs/configuration/reference/configuration_types.adoc#_authentication_data_source" >}}[Authentication Data Source]_ (optional, not overridable)
+
Where to get the access token from. Defaults to retrieve it from the `Authorization` header, the `access_token` query parameter or the `access_token` body parameter (latter, if the body is of `application/x-www-form-urlencoded` MIME type).
