...
* *`introspection_endpoint`*: _link:{{< relref "/docs/configuration/reference/types.adoc#_endpoint">}}[Endpoint]_ (mutually exclusive with oidc_discovery_endpoint, not overridable)
+
...
* *`oidc_discovery_endpoint`*: _link:{{< relref "/docs/configuration/reference/types.adoc#_endpoint">}}[Endpoint]_ (mutually exclusive with oidc_discovery_endpoint, not overridable)
+
The OIDC discovery endpoint of the OAuth2 authorization provider. At least the `url` must be configured. If the OAuth introspection URL is not known upfront, it can be fetched via https://openid.net/specs/openid-connect-discovery-1_0.html. The URL is a link:{{< relref "overview.adoc#_templating" >}}[template] and has the link:{{< relref "overview.adoc#_values" >}}[`Token`] string available that contains the raw token as sent from the client. The host part of this URL can't be templated for security reasons.
...
+
Configures the required claim assertions. Overriding on rule level is possible even partially. Those parts of the assertion, which have not been overridden are taken from the prototype configuration. If the OIDC discovery document is used, the list of issuers is optional here, as the issuer will be fetched from the discovery document.
...
.Configuration for Keycloak utilizing OIDC
====
[source, yaml]
----
id: keycloak
...
config:
  oidc_discovery_endpoint:
    url: http://keycloak:8080/realms/app/.well-known/openid-configuration
  assertions:
    # Note that no issuer is configured here, since it'll be taken automatically based on the .well-known
    algorithms:
      - RS256
----
====
...
* *`jwks_endpoint`*: _link:{{< relref "/docs/configuration/reference/types.adoc#_endpoint">}}[Endpoint]_ (mutually exclusive with oidc_discovery_endpoint, not overridable)
+
...
* *`oidc_discovery_endpoint`*: _link:{{< relref "/docs/configuration/reference/types.adoc#_endpoint">}}[Endpoint]_ (mutually exclusive with jwks_endpoint, not overridable)
+
The OIDC discovery endpoint of the JWT authorization provider. At least the `url` must be configured. If the JWKS URL is not known upfront, it can be fetched via https://openid.net/specs/openid-connect-discovery-1_0.html. The URL is a link:{{< relref "overview.adoc#_templating" >}}[template] and has the link:{{< relref "overview.adoc#_values" >}}[`JWT`] object available that contains the deserialized claims of the input JWT. The Claims have *not* been validated at this point. The host part of this URL can't be templated for security reasons.
