...
* *`authentication_data_forward`*: _link:{{< relref "/docs/configuration/reference/types.adoc#_authentication_data_forward_strategy" >}}[Authentication Data Forward Strategy]_ (optional, not overridable)
+
How to forward the extracted authentication data to the `identity_info_endpoint`. Defaults to forwarding it using exact the same way it was present in the request to heimdall.
...
This example shows how to configure this authenticator to work with an authentication system, which issues a cookie upon successful user authentication to maintain the authentication state. To reduce the communication overhead, it also makes use of `cache_ttl` to cache the response for 5 minutes if that time frame does not exceed the actual validity of the session represented by the cookie. Here, the authenticator will call the `http://my-auth.system/sessions/whoami` endpoint by also sending the `my_session` cookie to it.
...
In this configuration the authenticator extracts the token from the `Authorization` header and request the information about the corresponding user from the `http://my-auth.system/introspect` endpoint by sending the extracted token in the body of the request in a parameter named `token` application/x-www-form-urlencoded encoded.
====
