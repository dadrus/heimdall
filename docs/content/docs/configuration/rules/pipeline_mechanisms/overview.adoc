...
In simplest case a rule will just reuse a handler. In more complex cases a rule can reconfigure parts of it (More about rules configuration can be found link:{{< relref "../overview.adoc" >}}[here]). Which parts can be reconfigured are handler specific and described in the documentation of each handler.
...
This object contains information about the request being authenticated and has the following attributes and methods:
...
+
This method expects the name of a header as input and returns the value of the header as `string` if present in the HTTP request. If not present an empty string (`""`) is returned.
+

* *`Cookie(name)`*: _method_,
+
This method expects the name of a cookie as input and returns the value of it as `string` if present in the HTTP request. If not present an empty string (`""`) is returned.
