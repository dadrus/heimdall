...
This string is available in the OAuth2 Introspection Authenticator and contains the Token as received from the client.

=== JWT

This object is available in the JWT Authenticator and contains the unvalidated, deserialized claims of the Token directly from the client. This can be used to build dynamic OIDC Discovery URLs directly based on the clients token.
...
Proper error handling requires attention to the actual _link:{{< relref "/docs/configuration/reference/types.adoc#_errorstate_type" >}}[error type]_ available via `type(Error)`.
...
* `parseJWT` - Parses a string as JWT, returns the claims of the JWT as dict. Does not perform any validation or signature checks on the JWT.
+
Example: `{{ get (parseJWT .Token) "sub" }}` evaluates to the `iss` field of the JWT.
