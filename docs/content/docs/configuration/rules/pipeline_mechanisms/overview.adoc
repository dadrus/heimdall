...
Depending on the mechanism type, there can be an additional `config` property, which then holds the mechanism's specific configuration an `if` property, which specifies when a particular mechanism should be executed.
...
This object represents an error, which has been raised during the execution of the pipeline and is available in link:{{< relref "error_handlers.adoc">}}[Error Handler] `if` link:{{< relref "#_expressions#">}}[CEL expressions]. Following properties are available:
