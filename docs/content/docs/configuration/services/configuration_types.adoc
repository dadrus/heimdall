...
+
This property enables mapping between response/error types used by heimdall and the corresponding HTTP status codes. Each entry must be from the list below and contain exactly one property named `code`, which then defines the desired mapping:

** `accepted` - this type is used to signal, the matched decision pipeline has been executed successfuly, so the request can be forwarded to the upstream service. The response of that type results by default in `202 Accepted` HTTP code.
** `precondition_error` - this error type is used to signal, that e.g. an argument expected by some mechanism was not present in the request. Error of this type results by default in `400 Bad Request` HTTP code.
** `authentication_error` - this error is used to signal that none of the authenticators configured for the matched rule pipeline could authenticate the subject of the request. Error of this type results by default in `401 Unauthorized` HTTP code.
** `authorization_error` - this error is used to signal authorization errors. Error of this type results by default in `403 Forbidden` HTTP code.
...
** `communication_timeout_error` - this error is used to signal a timeout while communicating to a remote system during the execution of the pipeline of the matched rule. Error of this type results by default in `502 Bad Gateway` HTTP code.
** `communication_error` - this error is used to signal a communication error while communicating to a remote system during the execution of the pipeline of the matched rule. A DNS name resolution error would be e.g. such an error. Error of this type results by default in `502 Bad Gateway` HTTP code.
** `internal_error` - this error is used to signal an internal error and results by default in `500 Internal Server Error`.
