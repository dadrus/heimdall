...
** *`off`* - Reject requests with encoded slashes. This is the default behavior.
...
* **Authentication Stage:** A list of link:{{< relref "/docs/mechanisms/authenticators.adoc" >}}[authenticator] references, each using `authenticator` as the key followed by the required authenticator id. Authenticator steps may each contribute their own principal to the resulting subject, or they may target the same principal and thus form a fallback chain, where a subsequent step is only used if a previous one fails or determines that it is not responsible for the authentication data present in the request.
+
To express which principal an authenticator step should create, the `principal` property can be used. The following rules apply when multiple authenticator steps are defined:
+
** If the principal name is omitted, the step creates a default principal.
** When authenticator steps specify different principal names, they are all evaluated, and each successful authenticator contributes its principal to the resulting subject (multi-principal authentication).
** When authenticator steps specify the same principal name, they form a fallback chain: only the first authenticator that succeeds determines the principal.
+
[NOTE]
====
...
Both `A` and `B` will be executed. On success, `A` creates a principal named `default`, and `B` a principal named `custom`. If any of these steps fails, the entire authenticate stage fails.
====
+
...
Both steps establish the `default` principal, with `B` acting as fallback if `A` fails.
====
+
...
- authenticator: C
  principal: custom
----
`A` must succeed and establishes the `default` principal. Either `B` or `C` must succeed and establish the `custom` principal.
====
...
- finalizer: bar
  config:
    ...
    - X-Some-Data: {{ quote .Outputs.second.Payload.result }}
