...
+
Whether to allow backtracking if a request is matched based on the `path` expression, but the additional matching conditions (see below) are not satisfied. Defaults to `false`.
...
+
A https://github.com/gobwas/glob[glob expression], which should be satisfied by the path of the incoming request. Mutually exclusive with `path_regex`.
...
+
A https://github.com/gobwas/glob[glob expression] which should be satisfied by the host of the incoming request. Mutually exclusive with `host_regex`.
...
Same path expressions with different conditions may be defined by multiple rules in the same rule set.
