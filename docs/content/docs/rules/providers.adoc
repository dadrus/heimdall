...
+
If configured, heimdall starts validation and conversion webhooks for integration with the Kubernetes API server. The validation webhook validates applied `RuleSet` resources before they are made available for loading and reports configuration errors directly during admission. The conversion webhook translates between supported `RuleSet` schema versions; see also link:{{< relref "/docs/operations/migration.adoc" >}}[Upgrade & Migration].
+
The HTTP contract is documented in the link:{{< relref "/openapi/_index.adoc#tag/Kubernetes-Admission-Controller" >}}[API] documentation. For listener and deployment details, see link:{{< relref "/docs/operations/services.adoc#_kubernetes_webhook_service" >}}[Kubernetes Webhook Service].
+
[CAUTION]
====
The Kubernetes API server must trust the certificate used by the webhooks. When using heimdall's Helm chart, the https://github.com/dadrus/heimdall/blob/main/charts/heimdall/README.adoc#configuration[`webhooks.caBundle`] value must therefore be set or injected.

Although validation and conversion share the same heimdall listener, the Helm chart exposes them through separate Kubernetes `Service` resources. The webhook certificate must cover both service names. See link:{{< relref "/docs/operations/services.adoc#_kubernetes_webhook_service" >}}[Kubernetes Webhook Service] for details.
====
