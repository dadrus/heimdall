...
+
To let the Kubernetes API server use these controller, https://github.com/dadrus/heimdall/blob/main/charts/heimdall/README.adoc#configuration[`webhooks.caBundle`] must be set or injected. Otherwise, the Kubernetes API server won't trust the configured TLS certificate and won't use the corresponding endpoints.
