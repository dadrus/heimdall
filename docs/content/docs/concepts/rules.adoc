...
The values used for matching — host, path, scheme, and method — are typically extracted directly from the incoming HTTP request. If link:{{< relref "/docs/operations/services.adoc#_trusted_proxies" >}}[configured to do so], heimdall can instead rely on values from standard `X-Forwarded-Proto`, `X-Forwarded-Host`, `X-Forwarded-Uri`, and `X-Forwarded-Method` headers.
