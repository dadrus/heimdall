...
Elements in the static configuration set up the services, like link:{{< relref "/docs/services/decision.adoc" >}}[decision service], which basically define the entry points, heimdall will listen to, the observability capabilities, like link:{{< relref "/docs/observability/logging.adoc" >}}[logging], the supported link:{{< relref "/docs/mechanisms/overview.adoc" >}}[pipeline mechanisms], the link:{{< relref "/docs/rules/default_rule.adoc" >}}[default rule], as well as the link:{{< relref "/docs/rules/providers.adoc" >}}[rule providers] (these elements are not expected to change often).
...
In all cases, a single rule definition adheres to the schema defined in link:{{< relref "/docs/rules/configuration.adoc" >}}[rule configuration].
