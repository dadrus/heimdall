...
+
This step finalizes the execution of the pipeline and typically transform the information collected so far about the subject and the request into objects expected by the upstream service. The available options range from adding a simple header over a structured JWT, to driving specific protocols, e.g. to obtain a token required by the upstream service.
