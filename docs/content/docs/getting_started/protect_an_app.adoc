...
<5> These two lines define the `link:{{< relref "/docs/mechanisms/authenticators.adoc#_anonymous" >}}[anonymous]` authenticator named `anon`, which allows any request and creates a link:{{< relref "/docs/mechanisms/evaluation_objects.adoc/#_principal">}}[Principal] with its `ID` set to `anonymous`.
