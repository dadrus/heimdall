...
Rules are the entities, which allow you to define the authentication & authorization and error pipelines required by your upstream services. These are rules, which bring previously configured link:{{< relref "/docs/getting_started/mechanisms.adoc" >}}[mechanisms] to life.

You can compare the relation between mechanisms and rules to a relation between a catalogue at a car dealer and a real car, when you get it. So, mechanisms is what you can see and select in a catalogue to compile your car and the rule is your car with real components and behaviour. In that sense rules instanciate mechanisms with the ability to tune these for your needs.

== Rule Types

Heimdall supports two types of rules:

* The upstream specific rule, also called the regular rule. These rules are dynamic by nature and can come and go together with the upstream service defining these.
...
As written above, a rule assembles an individual pipeline by making use of previously defined mechanisms, which are executed, when the rule matches the request.
...
* optionally where to forward the request to upon successful execution of the authentication & authorization pipeline if heimdall is operated in proxy mode (regular rules only)
...
In principle, a rule set is just file with a list of rules with some additional meta information, like format version, name of the rule set and alike. Rule sets do also allow ordering of rules, e.g. with most specific matching expressions first allowing simpler matching expressions.
...
In principle, there is a need to differentiate between the defined rule and the effective rule. The first one is what you define in the rule set and the second one, the effective rule, is what is executed at runtime (comparable to polymorphism concept in programming languages). So, how does it work?
...
On the other hand, the definition of a regular, respectively upstream specific rule is then not a class deriving from the `BaseRule`, but from the `DefaultRule`. That way upstream specific rules are only required, if the behavior the default rule would not fit the given requirements of a particular service, respectively endpoint. So, if e.g. a rule requires only the authorization stage to be different from the default rule, it would result in something like shown in the snippet below.
