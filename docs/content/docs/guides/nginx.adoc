...
NOTE: The above configuration requires `trusted_proxies` to be configured, otherwise heimdall will not make use of the `X-Forwarded-Host` and `X-Forwarded-For` headers
====
