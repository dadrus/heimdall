...
* *`access_log_enabled`*: _boolean_ (optional)
+
Controls whether the per-request access log events (`TX started` and `TX finished`, see <<_access_log_events>>) are emitted by the HTTP and gRPC endpoints. Defaults to `true`. Set this to `false` if your traffic is fronted by another component (e.g. an ingress controller or service mesh) that already produces an access log and you want to avoid the duplicate volume.
+
.Disabling access log events.
====
[source, yaml]
----
log:
...
In addition to regular logs, heimdall emits access logs. The log level of these events in the log output is always `INFO`, regardless the configured log level. Emission of these events can be turned off via the `access_log_enabled` configuration option (see <<_configuration>>).
...
If the request comes from an intermediary (for example, an API Gateway) and heimdall is configured to trust that proxy (see link:{{< relref "/docs/operations/services.adoc#_trusted_proxies" >}}[`trusted_proxies` configuration]), the following fields are also included when the corresponding HTTP headers are present.
...
* *`cover_cache`*: _boolean_ (optional)
+
...
* Cache usage information, if `metrics.cover_cache` is enabled and the link:{{< relref "/docs/operations/cache.adoc#_noop_backend" >}}[Noop Cache Backend] is not configured.
...
See the link:{{< relref "/openapi/_index.adoc#tag/Profiling" >}}[API] documentation for details about the HTTP interface. For enabling profiling and configuring its listener, see the link:{{< relref "/docs/operations/services.adoc#_profiling_service" >}}[Profiling Service].
