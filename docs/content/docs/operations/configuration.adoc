...
* Environment variables use the prefix configured through `--env-config-prefix`. Defaults to `HEIMDALLCFG_`.
+
Only environment variables with this prefix are used as heimdall configuration and are subject to the same schema validation as the configuration file.
...
+
E.g. the `trusted_proxies` property of the link:{{< relref "/docs/operations/services.adoc#_main_service" >}}[main service] can be configured in a config file as
+
[source,yaml]
----
serve:
