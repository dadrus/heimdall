...
secret:        # or credentials, or master_key
  source: <source-name>
  selector: <selector>
...
Some heimdall features, like e.g., the generation of nonces, require symmetric key material to derive feature-specific keys from. The `master_key` top-level setting provides a common way to reference such key material and is configured as a link:{{< relref "#_secret_references" >}}[Secret Reference] :
...
This setting is only required if a configured feature needs it. For example, DPoP nonce generation and validation require a master key if nonce validation is enabled.

NOTE: Currently, symmetric keys are supported by the link:{{< relref "#_jwks" >}}[`jwks`] secret provider only.

=== Master Key Rotation

Master key rotation is supported by configuring a secret source that can expose multiple symmetric keys and by changing the `master_key.selector` to the new key.

Features using the master key include a reference to the key used for a cryptographic operation in the data they create. When such data is later validated, the feature can use this reference to resolve the corresponding key from the configured secret source. This allows old keys to remain usable during a rotation window, as long as they are still available from the secret source.

A typical rotation process is:

. Add the new key to the configured secret source while keeping the old key available.
. Change `master_key.selector` to the new key.
. Keep the old key available for at least the maximum lifetime of data created with that key, plus any deployment or configuration reload delay.
. Remove the old key after the rotation window has passed.

If the referenced key is no longer available, data created with that key can no longer be validated and is treated as invalid by the feature using it.

.Master key configuration with JWKS
====
Given the following JWKS file located under `/etc/heimdall/master-keys.jwks`:
...
you would configure the `master_key` property as follows:
...
secret_management:
  master_keys:
    type: jwks
    config:
      path: /etc/heimdall/master-keys.jwks
      watch: true

master_key:
  source: master_keys
  selector: key-2026-02
----
====

In this example, `key-2026-02` is the currently selected master key. Features using the master key can still resolve `key-2026-01` from the same secret source if they need to validate data created with that key.
