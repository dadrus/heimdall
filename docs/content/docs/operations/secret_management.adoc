...
Secret sources are configured under the top-level `secret_management` key. Each source has a unique, freely chosen name, a link:{{< relref "#_supported_secret_providers" >}}[secret provider] `type`, provider-specific `config`, and an optional `allow_in_rules` flag.
