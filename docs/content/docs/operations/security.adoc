...
Usage of external files can even allow you to rotate the configured secrets without the need to restart heimdall. Watching for secrets rotation is however disabled by default and can be enabled by setting the `secrets_reload_enabled` property to `true` on the top level of heimdall's configuration.

NOTE: Secret reloading is not yet supported at any places where secrets are used. Please consider the documentation of the corresponding configuration option for details.
