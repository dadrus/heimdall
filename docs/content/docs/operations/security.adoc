...
* Configuring the link:{{< relref "/docs/operations/services.adoc#_trusted_proxies" >}}[`trusted_proxies`] property to allow insecure networks (`0.0.0.0/0`, `0/0`, `0000:0000:0000:0000:0000:0000:0000:0000/0`, and `::/0`) is prohibited. See also link:{{< relref "security.adoc#_http_header" >}}[HTTP Header]. This enforcement can be disabled (not recommended) by starting heimdall with the `--insecure-skip-secure-trusted-proxies-enforcement` flag.
...
Heimdall applies independent limits to resources used while receiving, processing, forwarding, and responding to requests. These include request headers and bodies, concurrent requests, connections and HTTP/2 streams, response progress and, in proxy mode, upstream connections and response headers. The corresponding settings and their defaults are documented in link:{{< relref "/docs/operations/services.adoc" >}}[Services]. These limits protect different resources and do not replace each other.

Where a protection can be disabled, do so only if an equivalent trusted limit exists elsewhere in the request path. Size limits should account for the expected size of authentication credentials, cookies, tracing headers, and request bodies. Streaming workloads should rely on progress and idle limits.

== Request Security Considerations

=== External Authorization

When heimdall is used in link:{{< relref "/docs/concepts/operating_modes.adoc#_decision_mode" >}}[Decision Operation Mode], the component forwarding the request remains the enforcement point. It must fail closed if heimdall is unavailable, including admission overload, or cannot provide an authorization decision.

For Envoy `ext_authz`, ensure `failure_mode_allow` is set to `false`. Equivalent fail-open options in other integrations should likewise be disabled.

=== HTTP Header

If the `trusted_proxies` property is configured (see also the corresponding link:{{< relref "/docs/operations/services.adoc#_trusted_proxies" >}}[configuration options]), heimdall may use specific HTTP headers to build the URL and HTTP method for rule matching. In that case, the following logic applies:

* The HTTP scheme is taken from the `X-Forwarded-Proto` header.
* The HTTP host and port are taken from the `X-Forwarded-Host` header.
* The HTTP path is taken from the `X-Forwarded-Uri` header, which may also contain query parameters.
* The HTTP method is taken from the `X-Forwarded-Method` header.

If any of these values is missing, heimdall falls back to the corresponding value from the actual request. For example, if `X-Forwarded-Method` is not set, the HTTP method used to communicate with heimdall is used for rule matching and evaluation.

This means that if a client integrating with heimdall does not use these headers and does not drop them, a malicious actor could spoof them and potentially escalate privileges (depending on your rules). To reduce this risk, follow these practices:

* If possible, avoid using `trusted_proxies`. In that case, these headers cannot be spoofed. However, you lose the original HTTP scheme, host, and port information and cannot rely on them in your rules.
* If you use forwarded headers, configure and set all of them consistently: `X-Forwarded-Method`, `X-Forwarded-Proto`, `X-Forwarded-Host`, and `X-Forwarded-Uri`.
* Configure your edge proxy to overwrite `X-Forwarded-*` headers instead of appending values.
* If you cannot control which headers your integrating system sets, ensure it drops unused ones. For example, if the proxy forwarding requests to heimdall sets only `X-Forwarded-Proto` and `X-Forwarded-Host`, it should drop `X-Forwarded-Method` and `X-Forwarded-Uri`.

The link:{{< relref "/guides/proxies/_index.adoc" >}}[API Gateways & Proxies Guides] follow these practices and highlight where additional caution is required.

=== Request Path

Before rule lookup, heimdall applies the following path handling:

* Literal dot segments (`.` and `..`) delimited by `/` are removed, adjacent literal slashes are merged, and a trailing slash is preserved. For example, `/user/../admin` becomes `/admin`, and `/api//admin` becomes `/api/admin`.
* Percent-encoded https://datatracker.ietf.org/doc/html/rfc3986#section-2.3[RFC 3986] unreserved characters (`ALPHA`, `DIGIT`, `-`, `.`, `_`, and `~`) are treated equivalently during rule lookup. For example, `/api/%61dmin` is matched as `/api/admin`.
* Encoded dot-segment forms are detected case-insensitively, including forms that use encoded slash (`%2f`) or backslash (`%5c`) as separators, and are rejected before rule execution.
* URL-encoded slashes are handled according to `allow_encoded_slashes` in link:{{< relref "/docs/rules/regular_rule.adoc" >}}[regular rules]. The default `off` rejects them. `on` accepts and decodes them, while `no_decode` accepts them without decoding.

The path authorized by heimdall is the path resulting from these rules. In proxy mode, this path is also the basis for the request forwarded by heimdall, subject to any configured URL rewrite. In decision mode, the integrating proxy is responsible for ensuring that the request it forwards represents the same target that was evaluated by heimdall.

[CAUTION]
====
Authorization is reliable only if heimdall and the component that ultimately routes or serves the request interpret the effective request target consistently. Heimdall cannot reproduce every downstream-specific URI or routing semantic. Ambiguous request targets should therefore be rejected before authorization or handled equivalently by all trusted components involved.

A known example is semicolon-delimited path parameters. `/public/..;/admin` does not contain an RFC 3986 `..` dot segment, so heimdall can match it below `/public`. A downstream that removes the semicolon-delimited path parameter first can subsequently normalize the resulting `/public/../admin` to `/admin`. The request can then reach a different resource than the one authorized by heimdall. This conflict can affect both proxy and decision mode and is documented in https://github.com/dadrus/heimdall/security/advisories/GHSA-9q28-xj2j-qpv8[GHSA-9q28-xj2j-qpv8]. Until the interpretations are equivalent, such ambiguous paths must be rejected by a trusted component before authorization or by the downstream before dispatch.
====
...
In a typical production scenario, there is a need for proper key and certificate management. This is supported by heimdall via link:{{< relref "/docs/operations/secret_management.adoc" >}}[secret management], which allows configuring different secret sources baked by different secret providers to load and reload secrets, credentials, key material, and certificate bundles from.

When configuring the actual secret provider – which in a simple form could be a PEM file containing keys and certificates

* make use of key ids. Usage of key ids allows for seamless key rotation in setups which do not support or allow usage of secret management systems.

* you can and should configure not only the private key, e.g., for signature creation purposes, but also the corresponding certificate chain. This way your upstream services are able not only to verify the signatures of the signed objects for cryptographic validity, but also perform verification of the revocation status of used certificates and also their time validity. All of that is crucial for secure communication.
+
...
[
  {
    ...
    "optional": {
      ...
      "1.3.6.1.4.1.57264.1.6": "refs/tags/v0.17.22",
      "Bundle": {
      ...
      "Subject": "https://github.com/dadrus/heimdall/.github/workflows/release.yaml@refs/tags/v0.17.22",
      "githubWorkflowName": "release",
      "githubWorkflowRef": "refs/tags/v0.17.22",
...
Replace `<tag>` with the chart version (e.g., `0.16.22`). On success, Cosign outputs JSON similar to the container image example and exits with `0`.
...
Replace `<tag>` with the chart version (e.g., `0.16.22`), and use `--source-tag` matching the GitHub release. Please note that the `<tag>` of the chart image does not correspond to the heimdall release version. On success, `slsa-verifier` prints a success message and exits with `0`.
...
Replace `<release version>` with the specific version tag of the release (e.g., `v0.17.22`) that corresponds to the archive.
