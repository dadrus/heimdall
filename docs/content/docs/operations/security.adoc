...
In a typical production scenario, there is a need for proper key and certificate management. This is supported by heimdall via link:{{< relref "/docs/operations/secret_management.adoc" >}}[secret management], which allows configuring different secret sources baked by different secret backends to load and reload secrets, credentials, key material, and certificate bundles from.

When configuring the actual secret backend – which in a simple form could be a PEM file containing keys and certificates
