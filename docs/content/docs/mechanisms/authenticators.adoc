...
menu:
  docs:
    weight: 3
    parent: "Mechanisms"
description: Authenticators verify the authentication data transferred by requests and create principals out of that data. This page describes the available authenticator types in detail.
---
...
This authenticator just creates a principal and sets its `ID` to `anonymous` without doing anything else. You can overwrite the value of principal's id by using the optional `config` property.
...
* *`principal`*: _string_ (optional, overridable)
+
Enables setting the `ID` of the created link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_principal" >}}[Principal] object to a custom value.
...
This authenticator verifies the provided credentials according to the HTTP "Basic" authentication scheme, described in https://datatracker.ietf.org/doc/html/rfc7617[RFC 7617]. It does however not challenge the authentication, it only verifies the provided credentials and creates a link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_principal" >}}[Principal] object with its `ID` set to the configured user identifier if the authentication succeeds. Otherwise, it raises an error. The link:{{< relref "error_handlers.adoc#_www_authenticate" >}}["WWW Authenticate"] error handler mechanism can for example be used if the corresponding challenge is required.

To enable the usage of this authenticator, you have to set the `type` property to `basic_auth`.

Configuration using the `config` property is mandatory. Following properties are available:

* *`credentials`*: _link:{{< relref "/docs/operations/secret_management.adoc#_secret_references" >}}[Secret Reference]_ (mandatory, not overridable)
+
Reference to the credentials object to be used for principal user id and password verification. The credentials object must include a `user_id` and a `password` entries with password being a valid argon2id hashed password in the PHC format
+
[source, yaml]
----
# contents of a credentials object
client_id: Aladdin
client_secret: $argon2id$v=19$m=65536,t=3,p=4$VmRjaCtoa2...KK9SzDkg
----

* *`error_signaling`*: _BasicAuthErrorSignaling_ (optional, overridable)
+
Configures whether authentication failures should be signaled using the `WWW-Authenticate` response header with the `Basic` authentication scheme.
+
...
.Full configuration of Basic Auth authenticator
====
[source, yaml]
----
id: simple_auth
...
This authenticator raises an `authentication_error` on following conditions:

* no authentication data is present in the request,
* the response received from the `identity_info_endpoint` is either `401 Unauthorized`, or `403 Forbidden`,
* the `session_lifespan.active` property evaluates to `false`,
* the session associated with the authentication data is not yet valid or already expired.
...
* *`principal`*: _link:{{< relref "/docs/configuration/types.adoc#_principal" >}}[Principal]_ (mandatory, not overridable)
+
Where to extract the link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_principal" >}}[Principal] information from the identity info endpoint response.
...
config:
  identity_info_endpoint:
    url: https://identitytoolkit.googleapis.com/v1/accounts:lookup?key={{ secret "some_source" "google_api_key" }}
    headers:
...
* *`principal`*: _link:{{< relref "/docs/configuration/types.adoc#_principal" >}}[Principal]_ (optional, not overridable)
+
Where to extract the link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_principal" >}}[Principal] information from the introspection endpoint response. If not configured `sub` is used to extract the principal `ID` and all attributes from the introspection endpoint response are made available as `Attributes`.
...
* *`error_signaling`*: _link:{{< relref "/docs/configuration/types.adoc#_token_usage_error_signaling" >}}[Token Usage Error Signaling]_ (optional, overridable)
+
Configures whether failures should be signaled using the `WWW-Authenticate` response header according to https://www.rfc-editor.org/rfc/rfc6750[RFC 6750] and related RFCs.
...
* *`principal`*: _link:{{< relref "/docs/configuration/types.adoc#_principal" >}}[Principal]_ (optional, not overridable)
+
Where to extract the link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_principal" >}}[Principal] `ID` from the JWT, as well as which attributes to use. If not configured `sub` is used to extract the principal id and all attributes from the JWT payload are made available as attributes.
...
* *`trust_anchors`*: _link:{{< relref "/docs/operations/secret_management.adoc#_secret_references" >}}[Secret Reference]_ (optional, not overridable)
+
The reference to the trust anchors, to be used for the JWK certificate validation. Defaults to system trust store.
