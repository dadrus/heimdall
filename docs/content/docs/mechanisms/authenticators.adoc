...
+
Where to extract the principal information from the identity info endpoint response.
...
+
Where to extract the principal information from the introspection endpoint response. If not configured `sub` is used to extract the principal `ID` and all attributes from the introspection endpoint response are made available as `Attributes`.
...
* *`principal`*: _link:{{< relref "/docs/configuration/types.adoc#_principal" >}}[Principal]_ (optional, not overridable)
+
Where to extract the principal id from the JWT, as well as which attributes to use. If not configured `sub` is used to extract the principal id and all attributes from the JWT payload are made available as attributes.
