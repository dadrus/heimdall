...
So, e.g. if heimdall handles a request for `\https://my-service.local/foo` and runs into an error like described above, the HTTP response will have code `302 Found` and `Location` set to `\https://127.0.0.1:4433/self-service/login/browser?return_to=https%3A%2F%2Fmy-service.local%2Ffoo`.
...
.Agentic MCP gateway error response
====

The error handler below emits a JSON-RPC error payload suitable for scenarios in which heimdall acts as an MCP gateway.
...
[WARNING]
====
The `redirect` error handler is deprecated and will be removed in `v0.19.0` version. Migrate to the `generic` error handler instead.
...
[WARNING]
====
The `www_authenticate` error handler is deprecated and will be removed in `v0.19.0` version. Migrate to the `generic` error handler instead.
