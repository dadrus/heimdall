...
A typical scenario is getting specific attributes for later authorization purposes which are not known to the authentication system and thus were not made available in link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_subject" >}}[`Subject's`] `Attributes` property.

If the API responds with a 2xx HTTP response code, the prepared response is made available through the link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_outputs" >}}[`Outputs`] object under the corresponding pipeline step ID. Its payload is exposed through the `Payload` property and response headers can be accessed using the `Header(name)` method.
...
+
Every entry in the `items` map is rendered independently. The resulting key-value map is exposed as the `Payload` of this contextualizer's entry in the link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_outputs" >}}[`Outputs`] object. For example, an item named `foo` produced by a pipeline step with the ID `prepare` is available as `Outputs.prepare.Payload.foo`.
