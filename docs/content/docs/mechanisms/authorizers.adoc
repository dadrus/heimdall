...
If your authorization system provides a payload in the response, heimdall inspects the `Content-Type` header to prepare the payload for further usage, e.g. in payload verification expressions or by a link:{{< relref "#_local_cel" >}}[Local (CEL)] authorizer. If the content type either ends with `json` or is `application/x-www-form-urlencoded`, the payload is decoded; otherwise, it is made available as a simple string.

The prepared payload is available to the authorization expressions as link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_payload" >}}[`Payload`] and, after successful authorization, through the link:{{< relref "/docs/mechanisms/evaluation_objects.adoc#_outputs" >}}[`Outputs`] object under the corresponding pipeline step ID. It is exposed through the output's `Payload` property, while response headers can be accessed using its `Header(name)` method.
...
In this case, since an OPA response could look like `{ "result": true }` or `{ "result": false }`, heimdall makes the prepared response payload available under `Outputs["opa"].Payload`, with `"opa"` being the pipeline step ID in this example. Response headers returned by OPA can be accessed through `Outputs["opa"].Header(name)`.
