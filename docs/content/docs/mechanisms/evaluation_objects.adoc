...
`Results` exists solely to support migration of existing configurations across the breaking change to the `Outputs` structure introduced with v0.18.0. New configurations should use `Outputs`.
====
...
Templates can act on all objects described above (link:{{< relref "#_subject" >}}[Subject], link:{{< relref "#_outputs" >}}[Outputs], link:{{< relref "#_request" >}}[Request], link:{{< relref "#_payload" >}}[Payload] and link:{{< relref "#_values" >}}[Values]). Which exactly are supported is mechanism specific.
