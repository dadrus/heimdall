...
* `secret` - Allows using configured secrets (See also link:{{< relref "/docs/operations/secret_management.adoc" >}}[Secret Management])
+
Example: `{{ secret "some_source" "my_api_key" }}` references a key via `"my_api_key"` selector in `"some_source"` secret source.
+
NOTE: Usage of secrets from rules is only possible if the referenced secrets source allows that (see link:{{< relref "/docs/operations/secret_management.adoc#_secret_source_configuration" >}}[Secret Source Configuration] for more details).
...
NOTE: Please note that `.Subject.ID` and `.Subject.Attribute` is a syntactic sugar for `.Subject.default.ID`, respectively `.Subject.default.Attributes`.

NOTE: Please also note how the access to the `foo` query parameter is done. Since `.Request.URL.Query.foo` returns an array of strings, the first element is taken to render the value for the `foo_value` key.
