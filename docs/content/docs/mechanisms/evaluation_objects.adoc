...
The subject represents the authenticated entity for the current request — such as a user, service, or machine. It is heimdall’s normalized form of the principal produced by a successful authenticator.

Subjects are immutable objects passed between mechanisms. They are also ephemeral — they exist only for the lifetime of a single request.

Each subject has the following properties:

* *`ID`*: _string_
+
The unique identifier of the subject. Its value is taken from the principal id created by an authenticator.

* *`Attributes`*: _map_
+
A key-value map containing all known attributes about the subject. Its value is taken from the principal attributes created by an authenticator.
