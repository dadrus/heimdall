...
This object is similar to the `Outputs` object and represents a pipeline-execution-specific key value map. It is used by pipeline steps to store or read results of particular step executions. The step id is used as a key and the value is the corresponding result.

Unlike the `Outputs` object, each result is stored in a structured form containing at least a `Payload` property. For pipeline steps receiving their results from remote systems, the result additionally contains a `Headers` property preserving the response headers.
