...
NOTE: Before upgrading to v0.18.0 (to be released soon), migrate references to `Outputs` to the `Results` object described below. In v0.18.0, `Outputs` will adopt the same structure as `Results`, which would otherwise introduce a breaking change that cannot be migrated automatically. After all heimdall instances have been upgraded to v0.18.0, references to `Results` can be changed back to `Outputs`. v0.18.0 will deprecate the `Results` object.

=== Results
This object is similar to the `Outputs` object and represents a pipeline-execution-specific key value map. It is used by pipeline steps to store or read results of particular step executions. The step id is used as a key, and the value is the corresponding structured result.

Each result provides the following property and method:
...
* *`Header(name)`*: _method_
+
Returns the value of the response header with the given name as a `string`. Header names are matched case-insensitively. If the header is not present, or the result was not produced by a pipeline step communicating with a remote HTTP endpoint, an empty string (`""`) is returned. If a header appears multiple times in the response, the returned `string` is a comma separated list of all values.
...
[source, gotemplate]
----
{{ .Results.id_2.Payload }}
----

Response headers of results produced by remote HTTP endpoints can be accessed using:

[source, gotemplate]
----
{{ .Results.id_1.Header "X-Some-Header" }}
----
...
----
{
  ...
  "whatever": {{ .Results.whatever.Payload | quote }}
}
----
