...
This object represents a pipeline-execution-specific key-value map containing the results produced by executed pipeline steps. The pipeline step ID is used as the key.

Each output object provides the following property and method:

* *`Payload`*: _any_
+
Contains the result produced by the corresponding pipeline step. For pipeline steps communicating with remote HTTP endpoints, this is the prepared response payload.

* *`Header(name)`*: _method_
+
Returns the value of the response header with the given name as a `string`. Header names are matched case-insensitively.
+
If the header is not present, or the output was not produced by a pipeline step communicating with a remote HTTP endpoint, an empty string (`""`) is returned.
+
If a header appears multiple times in the response, the returned `string` is a comma-separated list of all values.
...
Outputs = {
  "id_1": { "Payload": ["a", "b"] },
  "id_2": { "Payload": { "foo": "bar", "baz": false } }
}
----

The payload can be accessed from a Go template using:

[source, gotemplate]
----
{{ .Outputs.id_2.Payload }}
----

Response headers of outputs produced by remote HTTP endpoints can be accessed using:

[source, gotemplate]
----
{{ .Outputs.id_1.Header "X-Some-Header" }}
----

or from a CEL expression using:
...
[WARNING]
====
Starting with v0.18.0, `Results` is a deprecated alias for link:{{< relref "#_outputs" >}}[`Outputs`].
Both names reference the same evaluation object and therefore expose exactly the same entries, properties, and methods.

`Results` was introduced in v0.17.21 as a migration aid for the breaking changes planned for v0.18.0. Starting with v0.18.0, it is a deprecated alias for `Outputs` and will be removed in v0.19.0.
====
...
`Values` are meant to be used as the parameter interface of a reusable mechanism definition,
allowing rules to provide rule-specific inputs without coupling the definition to their structure.
See link:{{< relref "/docs/mechanisms/mechanism_design.adoc" >}}[Designing Mechanism Definitions]
for details.

== Templating

Some mechanisms support templating using https://golang.org/pkg/text/template/[Golang Text Templates].
Templates can act on evaluation objects such as link:{{< relref "#_subject" >}}[Subject], link:{{< relref "#_outputs" >}}[Outputs], link:{{< relref "#_request" >}}[Request], link:{{< relref "#_payload" >}}[Payload] and link:{{< relref "#_values" >}}[Values]. Which exactly are supported is mechanism specific.
...
----
{
  ...
  "whatever": {{ .Outputs.whatever.Payload | quote }}
}
----
