...
<2> Here we configure secret sources to get key material from. In this particular case a secret source named `jwt_key_store` is baked by the `pem` backend, which loads the referenced pem file.
