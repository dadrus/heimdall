...
In most cases you must configure heimdall to trust your NGINX instances by setting link:{{< relref "/docs/operations/services.adoc#_trusted_proxies" >}}[`trusted_proxies`]. Exceptions are described in the sections below.
...
NOTE: Making use of the annotations described below requires setting `controller.allowSnippetAnnotations` to `true` and `config.annotations-risk-level` to `Critical` in the controller's global configuration.
