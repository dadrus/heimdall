...
location = /_auth {  <4>
  ...
  proxy_pass               http://heimdall:4455$request_uri; <6>
...
location = /_auth {
  internal;
  proxy_pass               http://heimdall:4455;  <1>
...
[source, yaml]
----
global-auth-url: "http://<heimdall service name>.<namespace>.svc.cluster.local:<port>" # <1>
...
server-snippet: |
  location = /_auth {
    ...
    proxy_pass               http://<heimdall service name>.<namespace>.svc.cluster.local:<port>$request_uri;
    proxy_pass_request_body  off;
    proxy_set_header         Content-Length   "";
...
[source, yaml]
----
nginx.ingress.kubernetes.io/auth-url: "http://<heimdall service name>.<namespace>.svc.cluster.local:<port>"
