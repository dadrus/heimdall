...
mechanisms:
  ...
  authorizers:
    - id: openfga_check # <3>
      type: remote
      config:
        endpoint: http://openfga:8080/stores/{{ .Values.store_id }}/check # <4>
        payload: | # <5>
          {
...
providers:
  file_system: # <10>
    ...
    watch: true
----
<1> Here we configure a link:{{< relref "/docs/operations/secret_management.adoc#" >}}[secret source] which loads the referenced pem file and makes the corresponding key material available. In this example, the key is used to sign the JWT heimdall is issuing.
...
<3> Here we define and configure a `link:{{< relref "/docs/mechanisms/authorizers.adoc#_remote" >}}[remote]` authorizer named `openfga_check`, which we're going to use for the actual authorization purposes in our rules.
<4> Here we define the endpoint to be used for the authorization checks. Most probably, you'll want to hard code your OpenFGA model id. Since, we're going to create the model, when we start our setup, we'll reference it in our rule via `store_id`.
+
...
+
<5> This is the definition of our payload to be sent to the check endpoint. As we don't know the model id as well, we'll configure it in our rule. The user will be taken from the `Subject` create by heimdall, and the relation and object will be specified in our rule.
+
In case of a successful response, the response from the check endpoint will look like `{"allowed": true}`. Otherwise, it will be `{"allowed": false}`. With the expression here, we perform the required verification.
...
<9> The following two lines define the `link:{{< relref "/docs/mechanisms/finalizers.adoc#_jwt" >}}[jwt]` finalizer. With the given configuration, it will create a jwt out of the subject object with standard claims, set the `sub` claim to the value of subject's ID, and sign it with the first key available in the referenced secret source.
...
. Create a file, named `signer.pem` with the following content. This is the PEM file with a private key, you've seen in the configuration of the secret source above.
+
[source, yaml]
----
