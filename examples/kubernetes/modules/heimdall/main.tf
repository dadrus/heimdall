...
resource "kubectl_manifest" "pod_monitor" {
  ...
  count = var.observability.metrics_enabled && var.observability.metrics_exporter == "prometheus" ? 1 : 0

  yaml_body = templatefile("${path.module}/manifests/pod_monitor.yaml", {
    namespace = helm_release.heimdall.namespace
  })
}
